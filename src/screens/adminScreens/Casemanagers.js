...
import { fetchCasemanager, addCaseManager, updateCaseManager } from "../../actions/caseManagersAction";
...
class CaseManager extends Component {
    ...
    static getDerivedStateFromProps(props, state) {
        if (props.caseManagers !== state.caseManagers) {
            return {
                caseManagers: props.caseManagers
            }
        }
        return null;
    }
    ...
    componentDidMount() {
        this.props.dispatch(fetchCasemanager())
    }
    ...
    handleSubmit = () => {
        ...
        let { mode, user_profile_id, first_name, last_name, user_name, password, confirm_password } = this.state
        let isError = false;
        if (mode === "ADD") {
            ...
            if (isError === false) {
                let caseManagerObj = {};
                caseManagerObj.id = this.props.caseManagers.length + 1
                caseManagerObj.first_name = first_name;
                caseManagerObj.last_name = last_name;
                caseManagerObj.user_name = user_name;
                caseManagerObj.password = "password";
                caseManagerObj.user_type = "CMR";

                this.props.dispatch(addCaseManager(this, caseManagerObj))
            ...
            if (isError === false) {
                let CaseManagerObj = {}
                CaseManagerObj.user_profile_id = user_profile_id;
                CaseManagerObj.first_name = first_name;
                CaseManagerObj.last_name = last_name;
                this.props.dispatch(updateCaseManager(this, CaseManagerObj, user_profile_id))
            }
        }
    ...
    render() {
        let filteredCaseManagers = this.props.caseManagers !== null && this.props.caseManagers.length > 0 ? this.props.caseManagers.filter((e) => e.userRole[0].role.name === "CaseManager") : null
            ...
            <div style={{ padding: "0px 20px 0px 20px" }}>
                <Grid container spacing={0}>
                    <Grid item xs={12} sm={12}>
                        <Toolbar style={{ justifyContent: "space-between", padding: "5px 20px 0px 0px" }}>
                            <Typography className={classes.textStyle}>Case Managers</Typography>
                            <TablePagination
                                rowsPerPageOptions={[20]}
                                component="div"
                                count={filteredCaseManagers !== null && filteredCaseManagers.length}
                    ...
                    <Grid item xs={12} sm={12} style={{ backgroundColor: "#fff", borderRadius: 5, boxShadow: "0ppx 0px rgba(0, 0, 0, 0.2)" }}>
                        <TableContainer style={{ backgroundColor: '#f2f2f2' }}>
                            <Table>
                                ...
                                <TableBody>
                                    {
                                        filteredCaseManagers !== null && filteredCaseManagers.length > 0 ? (filteredCaseManagers.map((item) => (
                                            ...
                                            :
                                            <TableCell colspan={4} style={{ textAlign: "center" }}>There are no case managers</TableCell>
                                    }
                                </TableBody>
                            </Table>
                        </TableContainer>
                ...
                <Dialog
                    ...
                    <DialogTitle id="form-dialog-title">
                        <Typography style={{ padding: "2% 5% 0 5%", fontFamily: "Futura-Heavy" }}>{this.state.mode === "ADD" ? "Add CaseManger" : "Update CaseManger"}</Typography>
                    </DialogTitle>
...
const mapStateToProps = (state) => {
    ...
    return {
        caseManagers: state.caseManagerReducer.caseManagers
    }
}
CaseManager.propTypes = {
    classes: PropTypes.object.isRequired,
};
export default compose(withStyles(styles), connect(mapStateToProps))(CaseManager);