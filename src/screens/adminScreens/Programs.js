...
import { fetchPrograms, addPrograms, updateProgram } from "../../actions/programAction";
...
class Programs extends Component {
    ...
    viewProgram = (item) => {
        ...
        this.props.history.push("/programs/categories", { 'id': item.id })
    }
    handleClose = () => {
        this.setState({
            dialogOpen: false,
    ...
    handleSubmit = (mode) => {
        ...
        if (isError === false) {
            let programObj = {};
            programObj.name = name;
            programObj.code = code;
            programObj.note = note;
            programObj.status = "ACTIVE"
            if (mode === "ADD") {
                this.props.dispatch(addPrograms(this, programObj))
            }
            else {
    ...
    render() {
        // console.log("*************************88" + this.state.programs)
            ...
            <div style={{ padding: "0px 20px 0px 20px" }}>
                <Grid container spacing={0}>
                    <Grid item xs={12} sm={12}>
                        <Toolbar style={{ justifyContent: "space-between", padding: "5px 20px 0px 0px" }}>
                            <Typography className={classes.textStyle}>Programs</Typography>
                            <TablePagination
                                rowsPerPageOptions={[20]}
                                component="div"
                                count={this.state.programs !== null && this.state.programs.length}
                    ...
                    <Grid item xs={12} sm={12} style={{ backgroundColor: "#fff", borderRadius: 5, boxShadow: "0ppx 0px rgba(0, 0, 0, 0.2)" }}>
                        <TableContainer style={{ backgroundColor: '#f2f2f2' }}>
                            <Table>
                                ...
                                <TableBody>
                                    {
                                        this.state.programs !== null && this.state.programs.length > 0 ? (this.state.programs.map((item) => (
                                            <TableRow key={item.id}>
                                                ...
                                                <TableCell align="left" className={classes.fontTableCellBody}>
                                                    {/* <IconButton onClick={() => this.handleClick(item, "EDIT")}><img src={Edit} height="20" alt="icon" /></IconButton > */}
                                                    <Button size="small" className={classes.viewButton} onClick={() => this.viewProgram(item)}>View</Button>
                                                </TableCell>
                                            </TableRow>)
                                        )) :
                ...
                <Dialog
                    ...
                    <DialogTitle id="form-dialog-title">
                        <Typography style={{ padding: "2% 5% 0 5%", fontFamily: "Futura-Heavy" }}>{this.state.mode === "ADD" ? "Add Program" : "Update Program"}</Typography>
                    </DialogTitle>
...
export default compose(withStyles(styles), connect(mapStateToProps))(Programs);
