...
import {
    ...
    DialogContent, DialogTitle, TextField, MenuItem, Toolbar, TableContainer
...
class ViewClient extends Component {
    ...
    submitForAssignProgram = () => {
        let { program_id, note, mode } = this.state
        ...
        if (isError === false) {
            ...
            assignedProgramObj.client_id = parseInt(this.props.location.state.id)
    ...
    submitForAssignStaff = () => {
        let { staff_id, staff_role_type, mode } = this.state
        let user_id = JSON.parse(localStorage.getItem('user')).user_id
        ...
        //     }
        // }
        if (staff_role_type === "" || staff_role_type === null) {
            this.setState({ staff_role_type_error: true })
            isError = true;
        }
        if (staff_id === "" || staff_id === null) {
            this.setState({ staff_id_error: true })
            isError = true;
        }

        if (isError === false) {
            let assignedProgramObj = {};
            assignedProgramObj.date = moment(new Date(), 'MM-DD-YYYY').format('YYYY-MM-DD 00:00');
            assignedProgramObj.client_id = parseInt(this.props.location.state.id)
            assignedProgramObj.staff_ref = staff_id
            assignedProgramObj.assigned_by_id = user_id
            assignedProgramObj.status = "ACTIVE"
            assignedProgramObj.role_code = staff_role_type
            if (mode === "ADD") {
    ...
    submitForSchedule = () => {
        let { date, rbt_id, session_start_time, session_end_time, mode } = this.state
        let user_id = JSON.parse(localStorage.getItem('user')).user_id
        let isError = false;
        if (date === null || date === "") {
            this.setState({ date_error: true })
        ...
        if (isError === false) {
            ...
            sheduleObj.client_id = this.state.client_id
    ...
    handleStaff = (event) => {
        this.setState({ staff_id: parseInt(event.target.value), isClear: true, staff_id_error: false });
    ...
    render() {
        ...
        // console.log("*******************client_schedules******************" + JSON.stringify(client_schedules))
        ...
        let filteredCaseManagers = this.props.clients.length > 0 ? this.props.clients.filter((e) => e.userRole[0].role.name === "CaseManager") : null
        ...
        return (
            <div style={{ marginLeft: 10, marginRight: 10 }}>
                <Toolbar style={{ justifyContent: "space-between", padding: "5px 20px 0px 0px" }}>
                    {filteredClient !== null && _.isEmpty(filteredClient) !== true && <Typography className={classes.textStyle}>{filteredClient.userProfile.first_name} {filteredClient.userProfile.last_name}</Typography>}
                    <div>
                        <Button size="small" className={classes.viewButton} onClick={() => this.assignProgram("ADD")}>Assign Program</Button>
                        <Button size="small" className={classes.editButton} onClick={() => this.assignStaff("ADD")}>Assign Staff</Button>
                    </div>
                </Toolbar>
                ...
                <Grid container className={classes.root} spacing={3}>
                    {/* <Grid item xs={4}>
                        <Card className={classes.card}>
                            <Typography style={{ backgroundColor: "#2EAAD4", padding: 10 }} className={classes.cardText} >Assigned-Program</Typography>
                            {
                                this.props.client_programs !== null && this.props.client_programs.length > 0 ?
                                    this.props.client_programs.map((item, index) => {
                                        return (
                                            <div>
                                                <Typography key={index} className={classes.cardTextStyle}>{index + 1}.{item.program.name}</Typography>
                                            </div>
                                        )
                                    })
                                    :
                                    <div>
                                        <Typography className={classes.cardTextStyle}>{"There are no assigned programs"}</Typography>
                                    </div>
                            }
                        </Card>
                    </Grid> */}
                    <Grid item xs={6}>
                        <Card className={classes.card} >
                            ...
                            {
                                filteredAssignedRbt !== null && filteredAssignedRbt.length > 0 ?
                                    filteredAssignedRbt.map((item, index) => {
                                        return (
                                            <div>
                                    ...
                                    : <div>
                                        <Typography className={classes.cardTextStyle}>{"There are no assigned rbts"}</Typography>
                                    </div>
                            }
                        </Card>
                    </Grid>
                    <Grid item xs={6}>
                        <Card className={classes.card} >
                            ...
                            {
                                filteredAssignedCaseManager !== null && filteredAssignedCaseManager.length > 0 ?
                                    filteredAssignedCaseManager.map((item, index) => {
                                        return (
                                            <div>
                                    ...
                                    :
                                    <div>
                                        <Typography className={classes.cardTextStyle}>{"There are no assigned case managers"}</Typography>
                                    </div>
                            }
                        </Card>
                    </Grid>
                ...
                <Dialog
                    ...
                    <DialogActions>
                        ...
                        <Button size="small" onClick={() => this.cancelSubmit()} variant="contained" color="secondary" className={classes.cancelButton}>Cancel</Button>
                    </DialogActions>
                </Dialog>
                <Dialog
                    maxWidth={"sm"}
                    ...
                    <DialogTitle id="form-dialog-title">
                        <Typography style={{ padding: "2% 5% 0 5%", fontFamily: "Futura-Heavy" }}>{this.state.mode === "ADD" ? "Assign Staff" : "Update Client"}</Typography>
                    </DialogTitle>
                    <DialogContent style={{ overflowY: "hidden" }}>
                        <Grid container justify="space-between" style={{ marginTop: -20, padding: "2% 5% 0 5%" }}>
                            {/* <Grid item xs={6} style={{ paddingRight: 8 }} >
                            ...
                            <Grid item xs={6} style={{ paddingRight: 8 }} >
                                <TextField
                                    ...
                                    select
                                    label="Staff Type"
                                    name="staff_role_type"
                                    value={this.state.staff_role_type}
                                    ...
                                    required
                                    fullWidth
                                    error={this.state.staff_role_type_error === true ? true : false}
                                    helperText={this.state.staff_role_type_error === true ? "Please select Staf Type" : false}
                                >

                                    <MenuItem key={"RBT"} value={"RBT"}>
                                        {"RBT"}
                                    </MenuItem>
                                    {(filteredAssignedCaseManager === null || filteredAssignedCaseManager.length === 0) &&
                            ...
                            <Grid item xs={6} style={{ paddingRight: 8 }} >
                                <TextField
                                    ...
                                    required
                                    fullWidth
                                    error={this.state.staff_id_error === true ? true : false}
                                    helperText={this.state.staff_id_error === true ? "Please select staff name" : false}
                                >
                                    {(this.state.staff_role_type === "RBT") ?
                                        ...
                                        :
                                        filteredCaseManagers !== null && filteredCaseManagers.map(item => (
                                            <MenuItem key={item.id} value={item.id}>
                                                {item.userProfile.first_name} {item.userProfile.last_name}
                                            </MenuItem>
                                        ))
                    ...
                    <DialogActions>
                        <Button size="small" onClick={() => this.submitForAssignStaff()} variant="contained" color="primary" className={classes.addButton}>Submit</Button>
                        <Button size="small" onClick={() => this.cancelSubmit()} variant="contained" color="secondary" className={classes.cancelButton}>Cancel</Button>
                    </DialogActions>
                </Dialog>
                            ...
                            <Grid item xs={6} style={{ paddingRight: 8 }} >
                                <MuiPickersUtilsProvider utils={DateFnsUtils}>
                                    {/* <KeyboardTimePicker
                                        ...
                                        helperText={this.state.session_end_time_error === true ? "Please enter session end Time" : false}
                                    /> */}
                                    <MuiPickersUtilsProvider utils={DateFnsUtils}>
                                        <KeyboardTimePicker
                                            margin="normal"
                        ...
                        {this.state.allEvents !== null && this.state.allEvents.length !== undefined && this.state.allEvents.map((e) => {
                            return (
                                <DialogContent dividers>
                                    <div>
                                        <EditIcon onClick={() => this.shedule("EDIT", e)} style={{ width: 25, height: 25, color: "#EACE09", position: "absolute", right: 10, fontWeight: 'bold' }}></EditIcon>
                                        <div className={classes.eventNameRow}>
                                            <EventIcon style={{ width: 20, height: 20, color: "#6E83B7" }} />
